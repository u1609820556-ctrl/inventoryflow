...
import { useImportarExcel, type DatosProveedorImportar } from '@/hooks/useImportarExcel';
import { showSuccess, showError, showWarning } from '@/components/ui/Toast';

interface ImportarExcelProveedoresProps {
  isOpen: boolean;
  onClose: () => void;
...
export default function ImportarExcelProveedores({ isOpen, onClose, onImportSuccess }: ImportarExcelProveedoresProps) {
  const {
    loading,
    error,
    preview,
  ...
  const handleFileSelect = useCallback(async (file: File) => {
    try {
      ...
      if (tipo !== 'proveedores') {
        showWarning('Este archivo parece ser de PRODUCTOS. Use el importador de productos.');
        setArchivo(null);
        return;
      }

      setDatosParseados(datos);
      const result = validarDatos(datos, 'proveedores');
      setValidacion(result);
      generarPreview(datos, 'proveedores', 5);
      setStep(2);
    } catch (err) {
  ...
  const handleImportar = useCallback(async () => {
    ...
    setImportando(true);
    try {
      const res = await importar(datosParseados, 'proveedores');
      ...
      const message = err instanceof Error ? err.message : 'Error al importar proveedores';
      showError(message);
    } finally {
      setImportando(false);
    }
        ...
        <div className="flex items-center justify-between p-6 border-b border-[#E2E2D5]">
          <div className="flex items-center gap-3">
            ...
            <h2 className="font-serif text-xl md:text-2xl font-bold text-[#064E3B]">
              Importar Proveedores (Excel/CSV)
            </h2>
          </div>
          <button
            onClick={onClose}
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {['nombre', 'email', 'telefono', 'direccion'].map((col) => (
                    <span
                      key={col}
                    ...
                    <table className="min-w-full divide-y divide-[#E2E2D5]">
                      <thead className="bg-[#F9FAFB]">
                        <tr>
                          ...
                          <th className="px-4 py-3 text-left text-xs font-medium text-[#6B7280] uppercase">Teléfono</th>
                          <th className="px-4 py-3 text-left text-xs font-medium text-[#6B7280] uppercase">Dirección</th>
                        </tr>
                      </thead>
                      <tbody className="bg-white divide-y divide-[#E2E2D5]">
                        {(preview as DatosProveedorImportar[]).map((item, i) => (
                          <tr key={i}>
                            <td className="px-4 py-3 text-sm text-[#374151]">{item.nombre || '-'}</td>
                            <td className="px-4 py-3 text-sm text-[#374151]">{item.email || '-'}</td>
                            <td className="px-4 py-3 text-sm text-[#374151]">{item.telefono || '-'}</td>
                            <td className="px-4 py-3 text-sm text-[#374151] truncate max-w-[150px]">{item.direccion || '-'}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
              ...
              {resultado.exito > 0 ? (
                <>
                  ...
                  <p className="text-lg text-[#6B7280]">
                    Se importaron <span className="font-bold text-[#064E3B]">{resultado.exito}</span> proveedores exitosamente
                  </p>
                </>
              ) : (
                <>
          ...
          {(loading || importando) && (
            <div className="flex flex-col items-center justify-center py-8">
              ...
              <p className="text-[#6B7280]">
                {importando ? 'Importando proveedores...' : 'Procesando archivo...'}
              </p>
              {progreso > 0 && (
